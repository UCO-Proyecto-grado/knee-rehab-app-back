...
from personas.app.shared.db.base import Paciente
from personas.app.schemas.acceso_personal.paciente_schema import PacienteOut
from personas.app.shared.utils.logging_config import get_logger
from personas.app.shared.utils.log_messages import LogMessages
import uuid
...
def get_paciente_by_id(db: Session, paciente_id: uuid.UUID) -> PacienteOut:
...
def get_pacientes_by_sede_id(db: Session, sede_id: uuid.UUID):
    logger.info(f"{LogMessages.Paciente.FETCH_BY_SEDE_ID} - ID: {sede_id}")
    try:
        pacientes = db.query(Paciente).filter(Paciente.id_sede == sede_id).all()
        return pacientes
    except Exception as e:
        logger.error(f"{LogMessages.Paciente.FETCH_BY_SEDE_ID_FAIL} - Error: {str(e)}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error interno al buscar pacientes")
